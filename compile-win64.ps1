...
<#
.SYNOPSIS
    Compiles inspectcode-tui into a standalone Windows binary with Nuitka.

.DESCRIPTION
    Produces a self-contained --standalone build (no Python install needed on
    the target machine). Output: dist\inspectcode-tui\inspectcode-tui.exe plus
    its DLLs, and a zipped dist\inspectcode-tui-vX.Y.Z-win64.zip ready to hand out.
#>
...
$root    = $PSScriptRoot
$entry   = Join-Path $root "src\inspectcode_tui\__main__.py"
$initPy  = Join-Path $root "src\inspectcode_tui\__init__.py"
$outDir  = Join-Path $root "dist"
$distDir = Join-Path $outDir "inspectcode-tui"
...
Write-Host "Compiling inspectcode-tui v$version with Nuitka..." -ForegroundColor Cyan
...
& $python -m nuitka `
    ...
    --output-dir=$outDir `
    --output-filename=inspectcode-tui.exe `
    --company-name="Michael Blaess" `
    --product-name="inspectcode-tui" `
...
if (Test-Path $nuitkaDist) { Rename-Item -Path $nuitkaDist -NewName "inspectcode-tui" }

$elapsed = [int]((Get-Date) - $started).TotalSeconds
$exe     = Join-Path $distDir "inspectcode-tui.exe"
...
# der Empfaenger entpackt also direkt einen sauberen inspectcode-tui-Ordner
$zip = Join-Path $outDir "inspectcode-tui-v$version-win64.zip"
